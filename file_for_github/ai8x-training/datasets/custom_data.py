...
class custom_data(Dataset):
    ...
    def __create_info_df_csv(self):
        ...
        #         sys.exit()
            ...
            # digit_struct_data = custom_data.read_digit_mat(os.path.join(mat_file_path))
            ...
            self.info_df['img_width'], self.info_df['img_height'] = \
                zip(*self.info_df['img_name'].apply(lambda name: custom_data.get_image_size(
    ...
    def __gen_datasets(self):
        ...
        for _, row in self.info_df.iterrows():
            ...
            increase = round(square_bb_width * custom_data.expansion_ratio / 2.)
...
def custom_data_get_datasets(data, load_train=True, load_test=True, resize_size=(96, 96), fold_ratio=2,
                      simplified=False):
    ...
    if load_train:
        ...
        train_dataset = custom_data(root_dir=data_dir, d_type='train',
    ...
    if load_test:
        ...
        test_dataset = custom_data(root_dir=data_dir, d_type='test',
...
def custom_data_74_get_datasets(data, load_train=True, load_test=True):
    """ Returns SVHN Dataset with 96x96 images
    """
    return custom_data_get_datasets(data, load_train, load_test, resize_size=(74, 74), fold_ratio=1)


def custom_data_74_simplified_get_datasets(data, load_train=True, load_test=True):
...
datasets = [
    {
        'name': 'custom_data_number',
        'input': (3, 74, 74),
        'output': (1, 2),
        'loader': custom_data_74_get_datasets,
        'collate': custom_data.collate_fn
    },
    {
        'name': 'custom_data_number_simplified',
        'input': (3, 74, 74),
        'output': ([1]),
        'loader': custom_data_74_simplified_get_datasets,
        'collate': custom_data.collate_fn
    }
]
